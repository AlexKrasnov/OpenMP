...
void ParallelRK(double *tmp_res, double x0_iter, int n, double h, char *buf, int * buf_len, int * buf_scroll, size_t grainsize)
{
	...
	tbb::parallel_for(tbb::blocked_range<size_t>(0, n, grainsize), [K, tmp_res, n]
	(const tbb::blocked_range<size_t> &r)
	{
		for (size_t i = r.begin(); i != r.end(); i++)
		{
...
void ParallelAdams(double *tmp_res, double *results, double x0, int n, double h, char *buf, int * buf_len, int * buf_scroll, size_t grainsize)
{
	double * deltaY = new double[n];
	for (int i = 0; i < n; i++)
		deltaY[i] = 0;
...
void SerialRK(double *tmp_res, double x0_iter, int n, double h, char *buf, int * buf_len, int * buf_scroll)
{
	int i(0), eqlen(0);
	double * K = new double[4 * n];
	double * deltaY = new double[n];
...
void SerialAdams(double *tmp_res, double *results, double x0, int n, double h, char *buf, int * buf_len, int * buf_scroll)
{
	double * deltaY = new double[n];
	int i(0), d(0);
	for (i = 0; i < n; i++)
...
void ParallelCalculation(double *results, double x0, int n, int m, double h, char *buf, int * buf_len, int * buf_scroll, size_t grainsize)
{
	int tmp_size = m * n;
	...
	for (int i = 0; i < 3; i++)
	{
		ParallelRK(tmp_res + i * n, x0, n, h, buf, buf_len, buf_scroll, grainsize);
		for (int j = 0; j < n; j++)
	...
	for (int i = 4; i < m; i++)
	{
		ParallelAdams(tmp_res, results + i * n, x0, n, h, buf, buf_len, buf_scroll, grainsize);
		for (int j = 0; j < n; j++)
			for (int k = 0; k < 4; k++)
...
void SerialCalculation(double *results, double x0, int n, int m, double h, char *buf, int * buf_len, int * buf_scroll)
{
	int tmp_size = m * n;
	double *tmp_res = new double[tmp_size]; // временные результаты
	for (int i = 0; i < n; i++)
	...
	for (int i = 0; i < 3; i++)
	{
		SerialRK(tmp_res + i * n, x0, n, h, buf, buf_len, buf_scroll);
		for (int j = 0; j < n; j++)
			results[(i + 1) * n + j] = tmp_res[(i + 1) * n + j];
		x0 += h;
	}
	...
	for (int i = 4; i < m; i++)
	{
		SerialAdams(tmp_res, results + i * n, x0, n, h, buf, buf_len, buf_scroll);
		for (int j = 0; j < n; j++)
			for (int k = 0; k < 4; k++)
				if (k != 3) tmp_res[k * n + j] = tmp_res[(k + 1) * n + j];
				else tmp_res[k * n + j] = results[i * n + j];
...
int main(int argc, char * argv[])
{
	...
	int n(100); // количество уравнений
	tick_count t1, t2;
	double x0(0.0), xn(1.0), h(0.1); // координаты отрезка, шаг
	double dt1(0), dt2(0), boost(0) ; // время работы, ускорение
...
#pragma endregion
	...
	t1 = tick_count::now();
	SerialCalculation(results2, x0, n, m, h, buffer, buffer_length, buffer_scroll);
	t2 = tick_count::now();
	dt2 = (t2 - t1).seconds();
	cout << "\n----------------------------------------------------------\n";
	...
	ParallelCalculation(results1, x0, n, m, h, buffer, buffer_length, buffer_scroll, grainsize);
	t2 = tick_count::now();
	dt1 = (t2 - t1).seconds();
	cout << "\n----------------------------------------------------------\n";
